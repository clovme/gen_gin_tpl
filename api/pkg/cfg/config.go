...
type Redis struct {
	...
	Username string `ini:"username" json:"username" comment:"Redis 用户名，若无用户名则为空"`
	Password string `ini:"password" json:"password" comment:"Redis 连接密码，若无密码则为空"`
	DB       int    `ini:"db" json:"db" comment:"Redis 数据库索引，默认为 0"`
}

type Email struct {
	SMTPHost string `ini:"smtp_host" json:"smtp_host" comment:"邮件服务器地址(smtp.qq.com)"`
	SMTPPort int    `ini:"smtp_port" json:"smtp_port" comment:"邮件服务器端口"`
	Username string `ini:"username" json:"username" comment:"邮件地址"`
	Password string `ini:"password" json:"password" comment:"SMTP服务，在第三方客户端登录时，需要使用授权密码"`
	From     string `ini:"from" json:"from" comment:"发件人地址，通常与用户名相同"`
}
...
type Logger struct {
	...
	LogPath    string `ini:"log_path" json:"log_path" comment:"日志文件存放路径"`
...
// Captcha 验证码配置
type Captcha struct {
	Length     int      `ini:"length" json:"length" comment:"验证码长度"`
	NoiseCount int      `ini:"noise_count" json:"noise_count" comment:"噪点数量（图片上加多少个干扰点）"`
	Type       []string `ini:"type" json:"type" comment:"验证码类型\n; digit\t\t数字\n; alphaNum\t数字字母\n; chinese\t汉字\n; math\t\t算术运算"`
	Fonts      []string `ini:"fonts" json:"fonts" comment:"验证码字体名称，以下是系统字体，只能选择，不可更换\n; 3Dumb,actionj,ApothecaryFont,chromohv,Comismsh,DeborahFancyDress,DENNEthree-dee,Flim-Flam,RitaSmith,wqy-microhei"`
	ShowLine   []int    `ini:"show_line" json:"show_line" comment:"配置干扰线条样式\n; 0\t\t无干扰线\n; 2\t\t空心线（中空的曲线，像钢笔划出来的线条）\n; 4\t\t粘稠线（细长黏糊糊的线，比较流动性）\n; 8\t\t正弦波线（类似波浪线，波形起伏）\n; 2|4\t空心线+粘稠线，两种线叠加，增强干扰\n; 2|8\t空心线+正弦波线，中空曲线和波浪线叠加\n; 4|8\t粘稠线+正弦波线，黏糊线+波浪线，视觉干扰力较强\n; 2|4|8\t空心线+粘稠线+正弦波线，全体上线，满满的视觉干扰"`
}
...
type Config struct {
	...
	Email   Email   `ini:"Email" json:"Email"`
	Web     Web     `ini:"Web" json:"Web"`
	Logger  Logger  `ini:"Logger" json:"Logger"`
	Captcha Captcha `ini:"Captcha" json:"Captcha"`
