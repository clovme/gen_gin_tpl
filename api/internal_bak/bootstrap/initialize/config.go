...
// 返回值：
//   - 无
func _setConfig[T comparable](key *T, value, defaultValue T, s em_status.Status) {
...
func _setConfigCache(key string, value, defaultValue any, s em_status.Status) {
	if !s.Is(em_status.Enable) {
...
// 返回值：
//   - 无
func _setByteConfig(key *[]byte, value, defaultValue []byte, s em_status.Status) {
	if !s.Is(em_status.Enable) {
		*key = defaultValue
		return
	}
...
func initializationSystemConfig(query *query.Query) {
	...
	for _, cfg := range configs {
		...
		case constants.ContextIsEncrypted: // 是否开启加密模式
			_setConfig(&variable.IsEnableEncryptedKey, cfg.Value == em_bool.True.Key(), cfg.Default == em_bool.True.Key(), cfg.Status)
