...
func (d *InitData) Permission() {
	...
	for i, route := range d.Router {
		...
		modelList = append(modelList, do_permission.Permission{Name: temp, Code: temp, PID: 0, Type: em_perm.Code(name), Uri: route.Path, Method: route.Method, Sort: i + 1, Description: route.Path})
	}
