...
func GrayTimeCheck() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		tStr := c.Query("_t")
		if tStr == "" {
			resp.JsonSafeCode(c, em_http.BadRequest, "缺少_t参数", nil)
			c.Abort()
			return
		}
		...
		if err != nil {
			resp.JsonSafeCode(c, em_http.BadRequest, "非法_t参数", nil)
			c.Abort()
			return
		}
		...
		if abs(now-timestamp) > 600 {
			resp.JsonSafeCode(c, em_http.Forbidden, "请求已过期或时间异常", nil)
			c.Abort()
			return
		}
