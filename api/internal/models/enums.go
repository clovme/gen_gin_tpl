...
type Enums struct {
	...
	Category    string        `gorm:"type:varchar(50);index;not null"` // 枚举分类
	Key         string        `gorm:"type:varchar(100);not null"`      // 枚举键（唯一标识）
	Name        string        `gorm:"type:varchar(100);not null"`      // 枚举名称（显示用）
	Value       int           `gorm:"type:int;not null"`               // 枚举值（数字）
	ValueT      int           `gorm:"default:0"`                       // 值类型
